...
describe("PUT /api/goals/:id/image route -> update goal image", () => {
  it("it should return 401 status code -> not authorized", async () => {
    const response = await request(app).put("/api/goals/1/image");
    expect(response.status).toBe(401);
    expect(response.body.msg).toBe("You are not authorized! Please login...");
  });
  it("it should return 400 status code -> id invalid format", async () => {
    const response = await request(app)
      .put("/api/goals/1/image")
      .set("Authorization", `Bearer ${token}`);
    expect(response.status).toBe(400);
  ...
  it("it should return 404 status code -> goal not found", async () => {
    const response = await request(app)
      .put("/api/goals/66317d8fb710891ebc8c32eb/image")
      .set("Authorization", `Bearer ${token}`);
    expect(response.status).toBe(404);
    expect(response.body.msg).toBe(
      "Goal with ID: 66317d8fb710891ebc8c32eb not found!"
    );
  });
  it("it should return 400 status code -> image file is missing", async () => {
    const response = await request(app)
      .put(`/api/goals/${goal1_id}/image`)
      .set("Authorization", `Bearer ${token}`);
    expect(response.status).toBe(400);
    expect(response.body.msg).toBe("Image file is missing!");
  });
  ...
  it("it should return 200 status code -> user image updated", async () => {
    const response = await request(app)
      .put(`/api/goals/${goal1_id}/image`)
      .set("Authorization", `Bearer ${token}`)
      .attach("image", `${__dirname}/files/post2.png`);
    expect(response.status).toBe(200);
  }); */
});
...
describe("DELETE /api/goals/:id/image route -> delete goal image", () => {
  it("it should return 401 status code -> not authorized", async () => {
    const response = await request(app).delete("/api/goals/1/image");
    expect(response.status).toBe(401);
    expect(response.body.msg).toBe("You are not authorized! Please login...");
  });
  it("it should return 400 status code -> id invalid format", async () => {
    const response = await request(app)
      .delete("/api/goals/1/image")
      .set("Authorization", `Bearer ${token}`);
    expect(response.status).toBe(400);
    expect(response.body.msg).toBe("ID: 1 - Invalid format!");
  });
  it("it should return 404 status code -> goal not found", async () => {
    const response = await request(app)
      .delete("/api/goals/66317d8fb710891ebc8c32eb/image")
      .set("Authorization", `Bearer ${token}`);
    expect(response.status).toBe(404);
    expect(response.body.msg).toBe(
      "Goal with ID: 66317d8fb710891ebc8c32eb not found!"
  ...
  it("it should return 400 status code -> no image", async () => {
    const response = await request(app)
      .delete(`/api/goals/${goal1_id}/image`)
      .set("Authorization", `Bearer ${token}`);
    expect(response.status).toBe(400);
    expect(response.body.msg).toBe("Goal does not have image to delete!");
  });
  it("it should return 200 status code -> user image updated", async () => {
    const response = await request(app)
      .put(`/api/goals/${goal1_id}/image`)
